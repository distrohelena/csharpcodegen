...
namespace cs2.cpp.tests {
    ...
    public class CPPCollectionExpressionAuditTests {
        /// <summary>
        /// Ensures a local array declaration initialized from a C# collection expression no longer produces conversion diagnostics.
        /// </summary>
        [Fact]
        public void WriteOutput_WithCollectionExpressionArrayDeclaration_DoesNotReportCollectionExpression() {
            string source = """
        ...
        /// <summary>
        /// Ensures jagged-array allocation with an omitted inner size lowers without unsupported-expression diagnostics.
        /// </summary>
        [Fact]
        public void WriteOutput_WithOmittedInnerArraySize_DoesNotReportOmittedArraySizeExpression() {
            string source = """
        ...
        static string RunConversion(string source, out JsonDocument report) {
            string rootPath = Path.Combine(Path.GetTempPath(), "cs2cpp-collection-tests", Guid.NewGuid().ToString("N"));
